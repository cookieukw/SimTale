...
- **Ranged** (bow, crossbow, firearm, ...): the Guard stops at range (about 7 blocks) instead of
  walking into melee distance.

A bow or crossbow still makes a **Hunter**, not a Guard — that check runs first, so nothing about
Hunter changed.

Because a new weapon (from another mod, or a future SimTale update) is not something SimTale can
predict by name, there are two ways to teach it about one: another mod calling
`WeaponCategoryRegistry.register("itemId", WeaponCategory.RANGED)` from its own code, or a
`simtale-weapons.json` file in the server's working directory listing `{"itemId": "MELEE"}` /
`{"itemId": "RANGED"}` entries by hand. Neither is required for weapons already built into Hytale.
...
| <img src="wiki/static/img/TownBell.png" width="32" align="absmiddle"/> Town Bell                        | anything                           | Rings the town curfew bell (10 uses), calling all villagers within 50 blocks to head home and sleep immediately |
...
| <img src="wiki/static/img/BirthdayCake.png" width="32" align="absmiddle"/> Birthday Cake                | a nearby villager                  | Celebrates with a villager, maximizing happiness (Mood EXCITED), filling hunger, and boosting affinity |
...
A handheld brass bell used as a village curfew signal. Ringing it plays a town bell chime in 3D audio and instructs all non-reaper villagers within a 50-block radius to drop their activities and head home to sleep immediately.
