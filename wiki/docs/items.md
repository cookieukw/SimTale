...
The one thing it does that the command cannot: it knows exactly which bed you mean. `housecheck` guesses by taking the nearest bed within 16 blocks, which stops being good enough the moment two houses share a wall.

## The three lenses

The Ledger, the Glass and the Journal are read-only views over data the mod already keeps. The first two open the same screens as `/simtale debugbeds` and `/simtale debugchests`, with the developer buttons removed.
