package com.cookieukw.SimTale.core.lifecycle;

import java.util.UUID;

/**
 * Componente de gravidez atribuído a um NPC feminino.
 * Contém dados sobre o pai, momento de início e duração.
 */
public class PregnancyComponent {

    /** Ticks por dia in-game (24000 ticks = 1 dia Hytale padrão). */
    public static final long TICKS_PER_DAY = 24000L;

    /** Duração padrão da gravidez em dias in-game. */
    public static final int DEFAULT_PREGNANCY_DAYS = 5;

    /** Se a NPC está grávida no momento. */
    public boolean pregnant = false;

    /** UUID do pai (pode ser um NPC ou um Player). */
    public UUID fatherId;

    /** Tick absoluto do mundo em que a gravidez começou. */
    public long startTick;

    /** Duração total em ticks. */
    public long durationTicks;

    /** Trimestre atual (1, 2 ou 3) — útil para comportamento futuro. */
    public int trimester = 0;

    public PregnancyComponent() {}

    /**
     * Inicia uma gravidez.
     *
     * @param fatherId  UUID do pai
     * @param worldTick tick atual do mundo
     */
    public void start(UUID fatherId, long worldTick) {
        this.pregnant = true;
        this.fatherId = fatherId;
        this.startTick = worldTick;
        this.durationTicks = DEFAULT_PREGNANCY_DAYS * TICKS_PER_DAY;
        this.trimester = 1;
    }

    /**
     * Inicia uma gravidez com duração customizada.
     */
    public void start(UUID fatherId, long worldTick, int durationDays) {
        start(fatherId, worldTick);
        this.durationTicks = durationDays * TICKS_PER_DAY;
    }

    /**
     * Retorna o progresso da gravidez (0.0 a 1.0).
     */
    public float getProgress(long currentTick) {
        if (!pregnant || durationTicks <= 0) return 0f;
        long elapsed = currentTick - startTick;
        return Math.min(1.0f, (float) elapsed / durationTicks);
    }

    /**
     * Verifica se a gravidez está concluída e o bebê deve nascer.
     */
    public boolean isReadyToBirth(long currentTick) {
        return pregnant && (currentTick >= startTick + durationTicks);
    }

    /**
     * Atualiza o trimestre baseado no progresso.
     */
    public void updateTrimester(long currentTick) {
        float progress = getProgress(currentTick);
        if (progress >= 0.66f) {
            trimester = 3;
        } else if (progress >= 0.33f) {
            trimester = 2;
        } else {
            trimester = 1;
        }
    }

    /**
     * Reseta o estado após o nascimento.
     */
    public void reset() {
        this.pregnant = false;
        this.fatherId = null;
        this.startTick = 0;
        this.durationTicks = 0;
        this.trimester = 0;
    }

    /**
     * Retorna quantos dias já se passaram desde o início da gravidez.
     */
    public int getElapsedDays(long currentTick) {
        if (!pregnant) return 0;
        return (int) ((currentTick - startTick) / TICKS_PER_DAY);
    }
}
